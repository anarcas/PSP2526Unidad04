...
public class HiloServidorSSLCookies implements Runnable {
    ...
    // Almacén de sesiones activas compartido entre todos los hilos (Otra manera de hacerlo con respecto el ejercicio anterior)
        ...
        try (SSLSocket s = this.socket; 
            ...
            if (peticion != null && (peticion.startsWith("GET") || peticion.startsWith("POST"))) {
                ...
                if (ruta.equals("/") && tipo.equals("GET")) {
                    ...
                    sesiones.put(sessionId,new Sesion());
                    ...
                    Sesion sesion=sesiones.get(sessionId);
                    
                    // Se aplica el método containsKey importante en el manejo de sesiones id                    
                    if (sessionId == null || !sesiones.containsKey(sessionId)) {
                        ...
                        if (tipo.equals("GET")) {
                            String info = "Último divisor usado: " + sesion.ultimoDivisor + " | Número de divisiones: "+sesion.numDivisiones;
                            respuesta = construirRespuesta(200, Paginas.html_division(info), sessionId);
                        } else if (tipo.equals("POST")) {
                            String divisor = cuerpo.toString().split("=")[1];
                            int num = Integer.parseInt(divisor);
                            int resultado = 10 / num;
                            sesion.ultimoDivisor=num;
                            sesion.numDivisiones++;
                            String info = resultado + " | Último divisor: " + sesion.ultimoDivisor+ " | Divisiones realizadas: "+sesion.numDivisiones;
                            respuesta = construirRespuesta(200, Paginas.html_division(info), sessionId);
                        }
                    }
